...
async function fetchHtml(url: string, attempts = 3) {
  ...
  for (let attempt = 0; attempt < attempts; attempt += 1) {
    try {
      ...
      if (attempt < attempts - 1) await sleep(650 * (attempt + 1));
    }
  }
...
async function mapBatches<T, R>(
  ...
  for (let i = 0; i < items.length; i += concurrency) {
    output.push(...(await Promise.all(items.slice(i, i + concurrency).map(fn))));
...
function daysBetween(
  ...
  return Math.round(
    (Date.UTC(b.year, b.month - 1, b.day, 12) - Date.UTC(a.year, a.month - 1, a.day, 12)) /
      86400000,
  );
}
...
function parseDetailDate(value: string | null) {
  ...
  const match = clean(value).match(
    /(\d{1,2})\.(\d{1,2})\.(\d{4})(?:\s+ob\s+(\d{1,2}):(\d{2}))?/i,
  );
...
function fieldAfterHeading($: ReturnType<typeof load>, label: string) {
  const heading = $("h1,h2,h3,h4,h5,h6")
    .filter(
      (_i, el) =>
        clean($(el).text()).toLocaleLowerCase("sl-SI") === label.toLocaleLowerCase("sl-SI"),
    )
    .first();
...
function inferCategory(title: string, content: string) {
  const t = title.toLocaleLowerCase("sl-SI");
  const c = content.toLocaleLowerCase("sl-SI");

  if (/vodstv|sprehod|ogled/.test(t)) return "Vodenje";
  if (/koncert|orkester|glasb|pevsk|jazz|rock|popevk/.test(t)) return "Glasba";
  if (/razstav|galerij|likovn|fotograf/.test(t)) return "Razstava";
  if (/delavnic|ustvarjal|akademij/.test(t)) return "Delavnica";
  if (/predstav|gledali|performans|muzikal|ples/.test(t)) return "Predstava";
  if (/tekma|šport|sport|košark|nogomet|koles|slalom|maraton|karting/.test(t)) return "Šport";
  if (/otrok|počitni|branje|knjiž|tabor/.test(t)) return "Za otroke";

  if (/voden|sprehod|ogled/.test(c)) return "Vodenje";
  if (/\bkoncert\b|\borkester\b|\bglasbeni\b|\bjazz\b/.test(c)) return "Glasba";
  if (/\brazstava\b|\bgalerija\b|\blikovn/.test(c)) return "Razstava";
  if (/\bdelavnica\b|ustvarjal/.test(c)) return "Delavnica";
  if (/\bpredstava\b|\bgledali/.test(c)) return "Predstava";
  if (/\btekma\b|\bšport\b|\bkošark/.test(c)) return "Šport";
  if (/\botrok\b|\bdružin/.test(c)) return "Za otroke";
  return "Dogodek";
}

function priceAndFree(content: string, title: string) {
  const explicitFree =
    /\bbrezplač/i.test(title) ||
    /\bvstop\s+prost/i.test(content) ||
    /\bbrezplač(?:en|na|no|ni)\b/i.test(content);
...
async function parseDetail(url: string) {
  ...
  const imageUrl =
    ...
    null;
  const locStatus =
    location && OUT_OF_AREA.some((pattern) => pattern.test(location)) ? "out_of_area" : "in_area";
...
Deno.serve(async (req: Request) => {
  ...
  if (
    ...
    Date.now() - new Date(source.last_synced_at).getTime() < 20 * 60 * 1000
  ) {
  ...
  collect(firstHtml);
  const pages = Array.from({ length: Math.max(0, maxPage - 1) }, (_v, i) => i + 2);
  await mapBatches(pages, 3, 300, async (page) => {
    try {
  ...
  if (!candidates.size) {
    return Response.json(
      { ok: false, error: "No current/future Visit Celje events discovered", pages_scanned: maxPage, warnings },
      { status: 502 },
    );
  }
  ...
  const { data: knownRows } = await supabase
    .from("events")
    .select("source_event_id,last_seen_at")
    .eq("source_id", source.id);
  const known = new Map(
    (knownRows ?? []).map((row) => [row.source_event_id, row.last_seen_at as string | null]),
  );
  ...
  const existingCandidates = ordered
    .filter(([url]) => known.has(slugFromUrl(url)))
    .sort(([urlA], [urlB]) => {
      const a = known.get(slugFromUrl(urlA));
      const b = known.get(slugFromUrl(urlB));
  ...
  const parsedResults = await mapBatches(work, 2, 350, async ([url]) => {
    try {
  ...
  if (published.length) {
    const minStart = new Date(
      Math.min(...published.map((event) => new Date(event.start_at).getTime())) - 86400000,
    ).toISOString();
    const maxEnd = new Date(
      Math.max(...published.map((event) => new Date(event.end_at ?? event.start_at).getTime())) + 86400000,
    ).toISOString();
    ...
    for (const event of published) {
      const direct = (existing ?? [])
        .map((candidate) => {
          ...
          if (
            !rangesOverlap(event.start_at, event.end_at, candidate.start_at, candidate.end_at) ||
            titleScore < 0.72
          ) return null;
          ...
          return {
            ...
            titleScore,
            venueScore,
            score: titleScore * 0.8 + venueScore * 0.12 + (sameStart ? 0.08 : 0.03),
          };
        })
      ...
      const series = (existing ?? [])
        ...
        .filter((item) => {
          ...
          return (
            event.event_type === "multiday" &&
            item.titleScore >= 0.82 &&
            candidateDate >= eventStart &&
            candidateDate <= eventEnd
          );
        });
      ...
      if (seriesSplit) {
        best = series.sort(
          (a, b) => b.titleScore + b.venueScore * 0.1 - (a.titleScore + a.venueScore * 0.1),
        )[0] ?? null;
      }
      if (!best) continue;
      ...
      const score =
        typeof best.score === "number"
          ? best.score
          : Math.min(1, best.titleScore * 0.9 + best.venueScore * 0.1);
      ...
      if (
        !seriesSplit &&
        event.end_at &&
        !best.candidate.end_at &&
        event.event_type !== "ongoing"
      ) {
        await supabase
          .from("events")
          .update({ end_at: event.end_at, event_type: event.event_type, updated_at: nowIso })
  ...
  return Response.json({
    ...
    new_selected: Math.min(newCandidates.length, MAX_DETAILS_PER_RUN),
    remaining_new_estimate: Math.max(0, newCandidates.length - MAX_DETAILS_PER_RUN),
